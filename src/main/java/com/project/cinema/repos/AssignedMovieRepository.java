...
public interface AssignedMovieRepository extends JpaRepository<AssignedMovie, Long> {
    ...
    @Query("select new com.project.cinema.dto.response.GetAssignedMovieDtoResponse(a.id, a.movie.id, a.movie.baslik, a.hall.id, a.hall.name, a.hall.capacity, a.startDateTime, size(a.reservedSeats) ) from AssignedMovie a where a.id= ?1")
